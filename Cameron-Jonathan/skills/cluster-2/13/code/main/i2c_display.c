...
void displayAveVolt(unsigned int* aveVolt)
{
  // Debugging
  int ret;
  ...
  {
    // Determine what number to display using some math
    displaybuffer[0] = ASCIILookupTable[getIndex('0') + ((*aveVolt / 1000) % 10)] | 0b0100000000000000;
    displaybuffer[1] = ASCIILookupTable[getIndex('0') + ((*aveVolt / 100 ) % 10)];
    displaybuffer[2] = ASCIILookupTable[getIndex('0') + ((*aveVolt / 10  ) % 10)];
    displaybuffer[3] = 0b0000000000111001;
...
void displayAveTemp(float* aveTemp)
{
  // Debugging
  int ret;
  int aveTempRound;
  ...
  while(1)
  {
    aveTempRound = round(*aveTemp);
    // Determine what number to display using some math
    displaybuffer[0] = ASCIILookupTable[getIndex('0') + ((aveTempRound / 10) % 10)];
    displaybuffer[1] = ASCIILookupTable[getIndex('0') + ((aveTempRound / 1 ) % 10)] | 0b0100000000000000;
    displaybuffer[2] = ASCIILookupTable[getIndex('0') + ((aveTempRound / 1  ) % 10)];
    displaybuffer[3] = 0b0000000000111001;
