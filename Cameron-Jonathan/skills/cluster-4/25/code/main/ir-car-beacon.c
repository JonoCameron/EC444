/* Infrared IR/UART beacons for crawler capture the flag!
   ...
   Edited Jonathan Cameron October 2020
*/
...
static xQueueHandle gpio_evt_queue_colour = NULL;
static xQueueHandle gpio_evt_queue_transmit = NULL;
...
static void IRAM_ATTR gpio_isr_handler_colour(void* arg){
  uint32_t gpio_num = (uint32_t) arg;
  xQueueSendFromISR(gpio_evt_queue_colour, &gpio_num, NULL);
}

// Button interrupt handler -- add to queue
static void IRAM_ATTR gpio_isr_handler_transmit(void* arg){
  uint32_t gpio_num = (uint32_t) arg;
...
static void button_init_colour() {
    ...
    gpio_isr_handler_add(GPIO_INPUT_IO_1, gpio_isr_handler_colour, (void*) GPIO_INPUT_IO_1);
    //create a queue to handle gpio event from isr
    gpio_evt_queue_colour = xQueueCreate(10, sizeof(uint32_t));
    //start gpio task
}

// Button interrupt init
static void button_init_transmit() {
    gpio_config_t io_conf;
    //interrupt of rising edge
    io_conf.intr_type = GPIO_PIN_INTR_POSEDGE;
    //bit mask of the pins, use GPIO36 here
    io_conf.pin_bit_mask = GPIO_INPUT_PIN_SEL_2;
    //set as input mode
    io_conf.mode = GPIO_MODE_INPUT;
    //enable pull-up mode
    io_conf.pull_up_en = 1;
    ...
    //gpio_install_isr_service(ESP_INTR_FLAG_LEVEL3);
    //hook isr handler for specific gpio pin
    gpio_isr_handler_add(GPIO_INPUT_IO_2, gpio_isr_handler_transmit, (void*) GPIO_INPUT_IO_2);
    //create a queue to handle gpio event from isr
...
void button_task_colour(){
  uint32_t io_num;
  while(1) {
    if(xQueueReceive(gpio_evt_queue_colour, &io_num, portMAX_DELAY)) {
...
void send_task(){
  while(1) {
      ...
      if(transmit_flag == 1){
        ...
        vTaskDelay(100 / portTICK_PERIOD_MS); // this is here so the watchdog doesn't get upset and continuously restart the board.
      }
      xSemaphoreGive(mux);
...
void id_task(){
  while(1) {
    for (int i = 0; i < (int) myID; i++) {    // printf("Action!\n");
...
// Timer task -- R (10 seconds), G (10 seconds), B (2 seconds)
...
void app_main() {
    ...
    xTaskCreate(button_task_colour, "button_task_colour", 1024*2, NULL, configMAX_PRIORITIES, NULL);
    xTaskCreate(button_task_transmit, "button_task_colour", 1024*2, NULL, configMAX_PRIORITIES, NULL);
