  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      else
      {
        ...
        if (rx_buffer[0] == '!' && rx_buffer[1] == '0')  // '!' is handled by the dish server.
        {
          // DEBUG
          printf("Got ! and 0 bytes! Stopping dish.\n");

          // Since received '0', stop the dish.
          *(conf.dish_active) = false;
        }
        else if(rx_buffer[0] == '!' && rx_buffer[1] == '1'){

          printf("Got  ! and 1 bytes! Starting dish.\n");

          // Since received '1', start the dish.
          *(conf.dish_active) = true;
        }

        if (rx_buffer[0] == '?' && rx_buffer[1] == '0') // '?' is handled by the remote control.
        {
          printf("Got a ? byte!\n");
        ...
        // This (~) is sent when the servo checks what state the dish is in and tells the RC that it doesn't have any pending instructions
        if(rx_buffer[0] == '~'){
          *(conf.pending) = false;
        }
...
int udp_client(const char* ip, uint16_t port, uint8_t* send_data, uint8_t* recv_data)
{
  int sockfd; 
	...
	int ret = sendto(sockfd, (const char *)send_data, strlen((const char*) send_data), 0, (const struct sockaddr *) &servaddr, sizeof(servaddr)); 
  close(sockfd);
